...
def main():
    ...
    # Fast movers: chargers and earbuds sell often, in decent quantities
    for i in range(1, 21):
        sales.record_sale(p1, 1, 3, 1600, days_ago(i))
    for i in range(1, 16):
        sales.record_sale(p2, 2, 4, 3700, days_ago(i))

    # Slow mover: bluetooth speaker barely sells, and stock is already low
    sales.record_sale(p3, 1, 1, 2900, days_ago(25))

    # Overstocked: HDMI cables and screen protectors have lots of stock, low sales
    sales.record_sale(p4, 1, 2, 750, days_ago(20))
    sales.record_sale(p6, 1, 3, 320, days_ago(18))

    # Laptop cooling pad: moderate, steady seller
